...
if (isset($_GET['id']) && !isset($_POST['update']))
    {
        ...
        $query = 'SELECT `first_name`, `email`, `pass`, `registration_date`, `last_name`, `user_id` FROM `users` WHERE `user_id` ='.$id ;
    ...
    elseif(isset($_POST['update']))
    {
        ...
        {
            $in = mysqli_real_escape_string($con, trim($_POST['last_name]']));
        }
        ...
        if (empty($errors))
        {
            $query = "UPDATE `users` SET `first_name`='$fn',
            `last_name`='$ln',
            `email`='$e',
            `pass`=SHA1('$p') WHERE `user_id`=$id";
            $r = @mysqli_query($con, $query);
            if ($r)
            {
                echo '<h1>Thank you!</h1>
                <p>User were updated!</p><p>
                <br/></p>';
                echo 'You will be redirect back withe 5 seconds &#128516;';
                header("refresh:5; url=users.php");
            }
            else
            {
                echo '<h1>System Error</h1>
                <p class="error"> You could not update due to a system error. we apologize for 
                any inconvenience.</p>';
        ...
        {
            echo '<h1> Error! </h1>
            <p class="error">The following error(s) occurred : <br/></p>';
